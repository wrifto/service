...
function createWorker(email, name, password, kyc) {
  ...
  const worker = {
    ...
    age: kyc?.age || 0,
    experience: kyc?.experience || 0,
    category: kyc?.category || '',
    categories: Array.isArray(kyc?.categories)
      ? kyc.categories
      : (kyc?.category ? [kyc.category] : []),
    hourlyRate: kyc?.hourlyRate || 0,
    kycVerified: !!(kyc?.age && (kyc?.category || (Array.isArray(kyc?.categories) && kyc.categories.length))),
    requests: [],
    specialty: kyc?.specialty || ''
  };
  saveWorkerDB(worker);
  return worker;
}
